    ...
    <div class="space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div class="flex flex-wrap items-end gap-3">
                <form method="get" action="{{ route('students.groups.schedule', ['groupName' => $groupName]) }}" class="flex flex-wrap items-end gap-2">
                    <div>
                        <label for="sch_date" class="block text-xs font-medium text-gray-500 mb-1">Дата недели</label>
                        <input id="sch_date" type="date" name="date" value="{{ $date }}" class="rounded-lg border border-gray-300 px-3 py-2 text-sm">
                    </div>
                    <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">Показать</button>
                </form>
