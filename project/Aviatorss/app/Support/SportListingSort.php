...
class SportListingSort
{
    public const FIELDS = ['name', 'created_at'];
    ...
    public static function applyToQuery(Builder $query, array $stack): void
    {
        ...
        foreach ($stack as $item) {
            ...
            if ($item['field'] === 'name') {
                $query->orderBy('sports.name', $dir);
            } elseif ($item['field'] === 'created_at') {
                $query->orderBy('sports.created_at', $dir);
            }
        }

        $tieDir = $stack[0]['order'] === 'asc' ? 'asc' : 'desc';
        $query->orderBy('sports.id', $tieDir);
    }
