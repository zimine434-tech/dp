...
@else
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-center gap-2">
            <label for="students_per_page_bottom_combobox_trigger" class="text-xs font-medium uppercase tracking-wide text-gray-500">Показывать по</label>
            <x-per-page-combobox :selected="(int)($perPage ?? 50)" input-id="students_per_page_bottom" />
        </div>
        <div class="sm:flex-1 sm:justify-end">
            @include('partials.compact-pagination', [
                'currentPage' => $studentListCurrentPage ?? 1,
                'lastPage' => $studentListLastPage ?? 1,
                'route' => 'students.index',
                'ariaLabel' => 'Страницы списка студентов',
            ])
        </div>
    </div>
    </div>
