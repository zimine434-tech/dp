    ...
    <div class="mx-auto max-w-3xl pb-64">
        <div class="overflow-visible rounded-lg bg-white shadow-lg">
            ...
            <div class="bg-gradient-to-r from-blue-500 to-blue-600 px-4 sm:px-6 py-4 sm:py-6 text-white">
                <h1 class="text-xl sm:text-2xl font-bold">Создание новой команды</h1>
                <p class="text-blue-100 mt-1 text-sm sm:text-base">Заполните форму для создания команды</p>
            </div>

            <!-- Форма -->
            <form action="{{ route('teams.store') }}" method="POST" class="p-4 sm:p-6 space-y-4 sm:space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <input
                        ...
                        value="{{ old('name') }}"
                        required
                        class="wysiwyg {{ $fieldClass }} @error('name') border-red-500 ring-1 ring-red-200 @enderror"
                ...
                <div>
                    ...
                    <x-teacher-sport-combobox :sports="$sports" :selected="old('sport_id')" name="sport_id" />
                </div>
