...
@if($competition->status === 'upcoming')
    <div class="rounded-lg bg-white p-4 shadow-md sm:p-5">
        ...
        <div id="competition-applications-empty" class="rounded-md border border-dashed border-gray-200 bg-gray-50 py-6 text-center {{ $pendingApps->isEmpty() ? '' : 'hidden' }}">
            <p class="text-xs text-gray-500">Входящих заявок пока нет</p>
        </div>
        <div id="competition-applications-table-wrap" class="overflow-x-auto rounded-md border border-gray-200 {{ $pendingApps->isEmpty() ? 'hidden' : '' }}">
                <table class="min-w-full divide-y divide-gray-200 text-xs sm:text-sm">
                    ...
                    <tbody id="competition-applications-tbody" class="divide-y divide-gray-200 bg-white">
                        @foreach($pendingApps as $application)
                            @php $student = $application->student; @endphp
                            <tr class="align-middle" data-application-student-id="{{ (int) ($student?->id ?? 0) }}">
