@php
    ...
    $newListStack = TrainingSessionListingSort::normalizeListStack(
        TrainingSessionListingSort::cycleTableColumn($listSortStack, $field, $defaultOrder)
    );
    $href = TrainingSessionListingSort::listingUrl(
        $listingRoute,
        $baseListingParams,
        $cardsSortStack,
        $newListStack,
    ...
    $activeOrder = TrainingSessionListingSort::orderForField($listSortStack, $field);
    $priority = null;
...
<a href="{{ $href }}" @if(filled($listingAjaxAttr)) {{ $listingAjaxAttr }}="1" @endif class="inline-flex cursor-pointer items-center hover:text-gray-900" title="Нажмите: по возрастанию → по убыванию → без сортировки">
    <span>{{ $label }}</span>
