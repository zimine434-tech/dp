...
@section('content')
    ...
    <div class="space-y-6">
        <div class="flex items-start justify-between gap-3">
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Мои тренировки</h1>
            <a
        ...
        <div class="flex flex-col gap-3 lg:flex-row lg:items-stretch lg:gap-4">
            <div class="flex min-w-0 flex-1 flex-col">
                ...
                <form method="GET" action="{{ route('training-sessions.student.my') }}" class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
                    <input type="hidden" name="page" value="1">
                    <input type="hidden" name="view" id="ts_my_filter_view" value="{{ $studentView }}">
                    <input type="hidden" name="per_page" id="ts_my_filter_per_page" value="{{ $perPage ?? 50 }}">
                    @if($filter !== 'all')
                    ...
                    <div class="flex flex-col gap-4 lg:flex-row lg:flex-nowrap lg:items-end lg:gap-4">
                        <div class="w-full min-w-0 shrink-0 lg:w-48 xl:w-52">
                            ...
                            <x-teacher-sport-combobox
                                ...
                                name="sport_id"
                                input-id="ts_my_sport"
                                empty-label="Все виды"
                                variant="filter"
                            />
                        </div>
                        <div class="min-w-0 flex-1">
                            <label for="ts_my_q" class="mb-1 block text-sm font-medium text-gray-700">Поиск по названию</label>
                            <input type="search" id="ts_my_q" name="q" value="{{ $lf['q'] ?? '' }}" maxlength="200" placeholder="Введите название..." autocomplete="off" class="block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm placeholder:text-gray-400 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500">
                        </div>
                        <div class="grid grid-cols-2 gap-3 sm:flex sm:shrink-0 sm:gap-3">
                            <div class="min-w-0 sm:w-40">
                                <label for="ts_my_date_from" class="mb-1 block text-sm font-medium text-gray-700">От</label>
                                <input type="date" id="ts_my_date_from" name="date_from" value="{{ $lf['date_from'] ?? '' }}" class="block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500">
                            </div>
                            <div class="min-w-0 sm:w-40">
                                <label for="ts_my_date_to" class="mb-1 block text-sm font-medium text-gray-700">До</label>
                                <input type="date" id="ts_my_date_to" name="date_to" value="{{ $lf['date_to'] ?? '' }}" class="block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500">
                            </div>
                        </div>
        ...
        <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
            ...
            <div class="flex flex-wrap gap-2">
                <a href="{{ $statusRoute('all') }}" class="filter-chip inline-flex items-center justify-center rounded-full border px-4 py-2 text-sm font-medium transition {{ $filter === 'all' ? 'border-blue-600 bg-blue-600 text-white shadow-sm hover:bg-blue-700' : 'border-gray-200 bg-gray-50 text-gray-800 hover:bg-gray-100' }}">Все</a>
                <a href="{{ $statusRoute('upcoming') }}" class="filter-chip inline-flex items-center justify-center rounded-full border px-4 py-2 text-sm font-medium transition {{ $filter === 'upcoming' ? 'border-blue-600 bg-blue-600 text-white shadow-sm hover:bg-blue-700' : 'border-gray-200 bg-gray-50 text-gray-800 hover:bg-gray-100' }}">Запланированные</a>
                <a href="{{ $statusRoute('in_progress') }}" class="filter-chip inline-flex items-center justify-center rounded-full border px-4 py-2 text-sm font-medium transition {{ $filter === 'in_progress' ? 'border-blue-600 bg-blue-600 text-white shadow-sm hover:bg-blue-700' : 'border-gray-200 bg-gray-50 text-gray-800 hover:bg-gray-100' }}">Идут сейчас</a>
                <a href="{{ $statusRoute('completed') }}" class="filter-chip inline-flex items-center justify-center rounded-full border px-4 py-2 text-sm font-medium transition {{ $filter === 'completed' ? 'border-blue-600 bg-blue-600 text-white shadow-sm hover:bg-blue-700' : 'border-gray-200 bg-gray-50 text-gray-800 hover:bg-gray-100' }}">Завершенные</a>
                <a href="{{ $statusRoute('cancelled') }}" class="filter-chip inline-flex items-center justify-center rounded-full border px-4 py-2 text-sm font-medium transition {{ $filter === 'cancelled' ? 'border-blue-600 bg-blue-600 text-white shadow-sm hover:bg-blue-700' : 'border-gray-200 bg-gray-50 text-gray-800 hover:bg-gray-100' }}">Отмененные</a>
            </div>
        </div>
        ...
        @else
            <div class="rounded-lg bg-white px-6 py-12 text-center shadow-md">
                ...
                <h3 class="mt-2 text-sm font-medium text-gray-900">Нет записей на тренировки</h3>
                <p class="mt-1 text-sm text-gray-500">
                    Откройте список тренировок и нажмите «Записаться».
                    <a href="{{ route('training-sessions.index') }}" class="font-medium text-blue-600 hover:text-blue-800">Перейти к тренировкам</a>
                </p>
            </div>
        @endif
    </div>
...
(function () {
    ...
    const viewHidden = document.getElementById('ts_my_filter_view');
    const PER_PAGE_STORAGE_KEY = 'training_sessions_student_per_page';
    const perPageHidden = document.getElementById('ts_my_filter_per_page');
    const perPageSelect = document.getElementById('ts_my_per_page_select');
