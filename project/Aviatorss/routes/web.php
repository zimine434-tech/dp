...
Route::middleware(['auth', EnsureAllowedRoles::class])->group(function () {
    ...
    Route::middleware([EnsureAllowedRoles::class . ':student'])->group(function () {
        Route::get('/profile/portfolio', [StudentPortfolioController::class, 'index'])->name('profile.portfolio');
        Route::get('/profile/portfolio/competitions/{competition}', [StudentPortfolioController::class, 'show'])
        ...
        Route::get('/profile/participations/competitions', function (Request $request) {
            ...
            $listingFilters['q'] = \Illuminate\Support\Str::limit(trim((string) ($listingFilters['q'] ?? '')), 200, '');
            ...
            $searchQ = $listingFilters['q'];

            $allParticipations = $user->competitionParticipants()
                ->with(['competition.sport', 'competition.team', 'competition.location', 'competition.participants.team.sport'])
                ->get();

            $sportIds = collect();
            foreach ($allParticipations as $participantRow) {
                $c = $participantRow->competition;
                if (! $c || $c->status !== 'finished') {
                    continue;
                }
                if ($c->sport_id) {
                    $sportIds->push((int) $c->sport_id);
                }
                if ($c->isPersonalCompetition()) {
            ...
            $filteredCompetitions = $allParticipations
                ->filter(function ($p) use ($sportId, $dateFrom, $dateTo, $searchQ) {
                    $c = $p->competition;
                    if (! $c) {
                        return false;
                    }
                    ...
                    if ($searchQ !== '' && mb_stripos((string) $c->name, $searchQ) === false) {
                        return false;
                    }
                    if ($sportId && ! $c->matchesListingSportFilter($sportId)) {
                        return false;
                    }
            ...
            $sportsForFilter = Sport::query()
                ->whereIn('id', $sportIds->unique()->values())
                ->orderBy('name')
                ->get();

            if ($sportId !== null && ! $sportsForFilter->contains('id', $sportId)) {
                $listingFilters['sport_id'] = null;
            }
        ...
        Route::get('/profile/participations/trainings', function (Request $request) {
            ...
            $filteredRegistrations = $user->trainingRegistrations()
                ...
                ->get()
                ->filter(function ($r) use ($sportId, $dateFrom, $dateTo, $searchQ) {
                    $t = $r->training;
                    if (! $t || ! $t->isParticipantHistoryFinished()) {
                        return false;
                    }
                    if ($searchQ !== '' && mb_stripos((string) $t->title, $searchQ) === false) {
                        return false;
                    }
    ...
    Route::get('/competitions/photo-archive', [CompetitionController::class, 'photoArchive'])->name('competitions.photo-archive')->middleware(['auth', EnsureAllowedRoles::class]);
    ...
    Route::middleware(['auth', EnsureAllowedRoles::class . ':teacher'])->group(function () {
        ...
        // Мои страницы (только созданные текущим преподавателем)
    ...
    Route::middleware(['auth', EnsureAllowedRoles::class . ':teacher'])->group(function () {
        ...
        Route::get('/competition-categories', [CompetitionCategoryController::class, 'index'])->name('competition-categories.index');
        Route::post('/competition-categories', [CompetitionCategoryController::class, 'store'])->name('competition-categories.store');
        Route::put('/competition-categories/{category}', [CompetitionCategoryController::class, 'update'])->name('competition-categories.update');
        Route::delete('/competition-categories/{category}', [CompetitionCategoryController::class, 'destroy'])->name('competition-categories.destroy');
        ...
        Route::get('/competitions/{competition}/search-teachers', [CompetitionController::class, 'searchTeachers'])->name('competitions.search-teachers');
        Route::post('/competitions/{competition}/teacher', [CompetitionController::class, 'updateResponsibleTeacher'])->name('competitions.teacher.update');

        // Сохранение форм (вид формы и номер формы) для студентов в соревновании
        Route::post('/competitions/{competition}/forms', [CompetitionController::class, 'storeForms'])->name('competitions.forms.store');
        Route::post('/competitions/{competition}/form-types', [CompetitionController::class, 'storeFormType'])->name('competitions.form-types.store');
        Route::post('/competitions/{competition}/medical-admission', [CompetitionController::class, 'storeMedicalAdmission'])->name('competitions.medical-admission.store');
        ...
        Route::post('/competitions/{competition}/generate-named-application', [CompetitionController::class, 'generateNamedApplication'])->name('competitions.generate-named-application');
    });
